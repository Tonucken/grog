<?php
/** Modell för att hantera Grogs moduler 
* @package GrogCore */
class CMModules extends CObject {
	private $grogCoreModules = array('CGrog', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
	private $grogCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
	
	public function __construct() {parent::__construct();}

	/** Lista alla tillgängliga controllers och metoder */
	...
	/** Läs och analysera alla moduler */
	public function ReadAndAnalyse() {
		$src = GROG_INSTALL_PATH.'/src';
		if(!$dir = dir($src)) throw new Exception('Kunde inte öppna mappen.');
		$modules = array();
		while (($module = $dir->read()) !== false) {if(is_dir("$src/$module") && class_exists($module)) {$modules[$module] = $this->GetDetailsOfModule($module);}}
		$dir->close();
		ksort($modules, SORT_LOCALE_STRING);
		return $modules;
	}
  
	/** Hämta detaljer om modulerna */
	private function GetDetailsOfModule($module) {
		...
		if(class_exists($module)) {
			...
			$details['isGrogCore']	 	= in_array($rc->name, $this->grogCoreModules);
			$details['isGrogCMF'] 		= in_array($rc->name, $this->grogCMFModules);
	...
	/** Hämta detaljer om en moduls metoder */
	...
	public function Install() {
		...
		uksort($allModules, function($a, $b) {return ($a == 'CMUser' ? -1 : ($b == 'CMUser' ? 1 : 0));}
			);
		$installed = array();
