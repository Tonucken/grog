<?php
/** Kör requesten och identifiera controller, metod och argument
* @package GrogCore */
...
class CRequest {
	...
	/** Skapa url i rätt format */
	public function CreateUrl($url=null, $method=null, $arguments=null) {
		if(!empty($url) && (strpos($url, '://') || $url[0] == '/')) {return $url;}
		if(empty($url) && (!empty($method) || !empty($arguments))) {$url = $this->controller;}
		if(empty($method) && !empty($arguments)) {$method = $this->method;}
		
		$prepend = $this->base_url;
		if($this->cleanUrl) { ; } 
		elseif ($this->querystringUrl) {$prepend .= 'index.php?q=';}
		else {$prepend .= 'index.php/';}
	...
	/** Kör aktuell url request och dela upp i controller, metod och argument */
	public function Init($baseUrl = null, $routing=null) {
		$requestUri = $_SERVER['REQUEST_URI'];
		$scriptName = $_SERVER['SCRIPT_NAME'];
		
		$i=0;		// Jämför REQUEST_URI och SCRIPT_NAME. Lämna resten av requesten om de matchar.
		$len = min(strlen($requestUri), strlen($scriptName));
		while($i<$len && $requestUri[$i] == $scriptName[$i]) {$i++;}
		$request = trim(substr($requestUri, $i), '/');
		
		$queryPos = strpos($request, '?');
		if($queryPos !== false) {$request = substr($request, 0, $queryPos);}
		
		if(empty($request) && isset($_GET['q'])) {$request = trim($_GET['q']);}
		
		$routed_from = null;		// Kontrollera om url matchar mot routing table i 'grog/site/config.php'
